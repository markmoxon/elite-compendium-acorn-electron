# Elite Compendium for the Acorn Electron with 16K sideways RAM
...
The repository also contains the menu code and a build process for producing DSD and ADF disc images, as well as two SSD images for use with SD cards. The SSD images are produced by BeebAsm, and the DSD and ADF images are created using the [oaknut-disc Python tool](https://rob-smallshire.github.io/oaknut/disc/index.html), which can be installed using `pip install oaknut-disc` (pipx, uv and uvx are also supported).
...
Thanks to the Bitshifters for their help in building the musical version of BBC Micro Elite, and in particular Kieran Connell, Simon Morris and Negative Charge for the music player and ported music files. Huge thanks also to Wouter Hobers (haerfest on Stardot) for his original sideways RAM version, from which I have taken both inspiration and the sideways RAM loader code.
